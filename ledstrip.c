...
//    Send_LED_StartFrame();
    for (char led = 0; led < NUMBER_OF_LEDS; led++) {
         //Send_LED_Frame(0xFF, 0x00, 0xFF, 0x00);
        /*
        if (led == led_run) {
...
void updateLEDstripGroen() {
    //start frame
    Send_LED_StartFrame();
    for (char led = 0; led < NUMBER_OF_LEDS; led++) {
         Send_LED_Frame(0x06, 0x00, 0xFF, 0x00);
    }
    //stop frame
    Send_LED_EndFrame();
}

void updateLEDstripOranje() {
    //start frame
    Send_LED_StartFrame();
    for (char led = 0; led < NUMBER_OF_LEDS; led++) {
         Send_LED_Frame(0x06, 0xFF, 0x80, 0x00);
    }
    //stop frame
    Send_LED_EndFrame();
}

void updateLEDstripRood() {
    //start frame
    Send_LED_StartFrame();
    for (char led = 0; led < NUMBER_OF_LEDS; led++) {
         Send_LED_Frame(0x06, 0x00, 0x00, 0xFF);
    }
    //stop frame
    Send_LED_EndFrame();
}
